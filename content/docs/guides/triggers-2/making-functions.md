...
spawn move_if_active(int object) {
    if (object.item == 1) {
...
<code><span class="k">spawn</span> <span class="nf">move_if_active</span>(<span class="nl">object</span>) {
    <span class="k">if</span> (<span class="nl">object</span>.item == <span class="mi">1</span>) {
        <span class="c1">// move object.group to the player</span>
    }
}
</code>
</pre>
...
```
Now, we can use function $ \verb|horizontal\_move|$ to move any object $10\cdot n$ units horizontally, where the direction depends on whether $n$ is positive or negative.
